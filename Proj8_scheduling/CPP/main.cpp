...
private:
	...
	int totalJobTimes; // the total job times for all nodes.
	int procGiven;
	int procUsed;
	int currentTime;
	int maxProcUsed;
	int **adjacencyMatrix; // representing the input dependency graph
	int **scheduleTable; // to record the schedule
	//i = node's ID
	//jobTimeAry = adjacencyMatrix[i][i] // to store the time requirement for each job
	Node *OPEN;	// to store jobs that do not have any parents, by the # of dependents
	int *processJob;	// which job is being processed by processor[i].
	int *processTime;	// the processing time remain on processor
	int *parentCount;	// to store number of parents of each job
	//kidCount = adjacencyMatrix[i][0] // to store number of dependents of each job
	//jobDone adjacencyMatrix[0][i]  // to keep track which jobs remain in the graph
	int *jobMarked;	// to see if jobs are currently on Open or on proccessor
//____________________________________________________________
...
//____________________________________________________________
	void scheduling(ofstream &outFile1, ofstream &outFile2) {
		...
		while (allJobDone() < 1) {
			...
			printOPENList();
			while (OPEN->next && procUsed < procGiven) {
...
//____________________________________________________________
	Node *findSpot(Node *newNode) {
		Node *spot = OPEN;
		while (spot->next != NULL && adjacencyMatrix[spot->next->jobID][0] > adjacencyMatrix[newNode->jobID][0]) {
			spot = spot->next;
		}
		return spot;
	}
...
//____________________________________________________________
	Node *getFirstJob() {
		Node *frontJob = OPEN->next;
		OPEN->next = frontJob->next;
		return frontJob;
	}
//____________________________________________________________
...
//____________________________________________________________
	int checkCycle() {
		...
		if (!OPEN->next && allJobDone() <= 0 && allProcIdle>= 1) { return 1; }
		return 0;
	}
//____________________________________________________________
	int findDoneJob() {
